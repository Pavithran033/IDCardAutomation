...
namespace IDCardAutomation.Pages.Library
{
    public class LibraryClearanceRequestsModel : PageModel
    {
        private readonly IConfiguration _config;
        public LibraryClearanceRequestsModel(IConfiguration config) => _config = config;
...
ORDER BY NR.RequestedDate DESC", conn);
        ...
        public IActionResult OnPost()
        {
            ...
            string updatedBy = User.Identity?.Name ?? "Mr.Nandakumar";
            DateTime updatedDate = DateTime.Now;

            using var conn = new SqlConnection(_config.GetConnectionString("DefaultConnection"));
            conn.Open();

            // 1. Update Library clearance
            string sql = command == "approve"
                ? "UPDATE LibraryClearance SET Status = 'Approved', Remarks = @Remarks, UpdatedBy = @UpdatedBy, UpdatedDate = @UpdatedDate WHERE RequestID = @RequestID"
                : "UPDATE LibraryClearance SET Remarks = @Remarks, UpdatedBy = @UpdatedBy, UpdatedDate = @UpdatedDate WHERE RequestID = @RequestID";

            using (var cmd = new SqlCommand(sql, conn))
            {
                cmd.Parameters.AddWithValue("@Remarks", string.IsNullOrWhiteSpace(remarks) ? "None" : remarks);
